<template>
  <div class="flex flex-col gap-3">
    <div class="mb-5 text-4xl font-bold">
      {{ productStore.detailProductInfo?.name }}
    </div>
    <div class="mb-4 text-red-500 text-3xl font-bold">
      {{
        new Intl.NumberFormat().format(productStore.detailProductInfo?.price)
      }}
      VND
    </div>
    <div class="mb-2 border-p-[0.5px] border-gray-300 pb-3 whitespace-pre-line">
      {{ productStore.detailProductInfo?.description }}
    </div>
    <div class="mb-2">
    ...
    <div class="mb-2">
      ...
      <div class="flex gap-2">
        <div
          @click="productStore.chooseColor(color)"
          v-for="color in productStore.detailProductInfo?.color"
          :key="color.name"
          :style="{ backgroundImage: 'url(' + color.color_thumbnail + ')' }"
          :class="[
            productStore.choosedColor.name == color.name
    ...
    <div class="mb-2">
      ...
      <div class="flex flex-wrap gap-2">
        <button
          @click="productStore.chooseSize(size)"
          v-for="size in productStore.detailProductInfo?.size"
          :key="size.name"
          :disabled="!productStore.sizeAvailable(size)"
          :class="[
            productStore.choosedSize.name == size.name
              ? 'border-2 border-red-500'
              : 'border-[0.5px] border-gray-300',
            productStore.sizeAvailable(size)
              ? ''
    ...
    <div class="mb-2">
      ...
      <div class="text-red-500" v-if="productStore.errorMessage.length">
