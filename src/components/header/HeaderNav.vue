  ...
  <nav class="h-full w-full flex items-center justify-between">
    <div class="flex items-center h-full">
      ...
      <ul class="h-full flex items-center">
        <li v-for="category of categoryStore.categories" :key="category.id"
          class="text-lg h-full px-4 flex flex-col justify-center group cursor-pointer"
          @mouseenter="() => openCategoryMenu(category.id)" @mouseleave="headerState.openMenu = false">
          <span class="font-bold"> {{ category.name }} </span>
          <span class="w-full bg-transparent group-hover:bg-red-300 h-[3px]"></span>
        </li>
        <li class="text-lg h-full px-4 flex flex-col justify-center group cursor-pointer">
    ...
    <div class="flex items-end gap-4">
      <div @click="headerState.searching = true"
