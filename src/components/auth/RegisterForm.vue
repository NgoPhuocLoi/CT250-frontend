  ...
  <div class="py-5 bg-grey-lighter min-h-screen flex flex-col">
    <div class="container max-w-lg mx-auto flex-1 flex flex-col items-center justify-center px-2">
      <div class="px-6 rounded shadow-md border text-black w-full">
        <h1 class="mt-6 mb-8 text-4xl font-bold text-center">Đăng ký</h1>
        ...
        <Form @submit="createUser" :validation-schema="userSchema" class="mb-12">
          ...
          <div class="flex flex-col w-full">
            ...
            <div class="relative">
              <Field type="password" name="confirmPassword" id="confirmPassword" v-model="confirmPassword"
                class="block w-full p-3 rounded border border-gray-300 mt-3 mb-2" />
              <!-- show confirm password begin -->
              <div class="absolute right-4 top-[50%] translate-y-[-50%]">
                <img @click="
                  toggleShowPassword('confirmPassword', 'eyeConfirmPassword')
          ...
          <div class="text-center text-grey-dark mt-7">
            Đã có tài khoản?
            <RouterLink class="no-underline border-b border-blue-800 text-blue-800" to="/xac-thuc/dang-nhap">
              Đăng nhập
            </RouterLink>
          </div>
        </Form>
